...
const projectRecords: ProjectRecord[] = [
  ...
  },
  {
    ...
    order: 3,
    title: "中国联通聚合支付平台",
    eyebrow: "商户接入 × 聚合收银 × 交易对账",
    summary: "基于历史项目资料完成脱敏重构，围绕商户一次接入、多渠道收银、交易售后与对账查询，搭建可演示的聚合支付 MVP。",
    problem: "联通内部业务的支付方式与账务能力相对分散，商户需要分别处理不同渠道的接入、支付结果和售后流程；运营与商户也缺少统一的交易查询和账务核对视图。项目需要收敛沃支付、微信和支付宝等能力，形成从下单、收银、回调到退款、撤销、对账的完整支付链路。",
    role: "基于历史项目资料完成脱敏重构，负责梳理需求、业务规则和接口边界，完成 PRD、交互原型及 Spring MVC MVP 的产品与实现验证。",
    constraints: ["商户一次接入，平台负责屏蔽渠道与客户端环境差异", "支付回调必须验签、幂等处理并向商户异步通知", "退款、撤销、对账和查询需受订单状态与数据权限约束"],
    decisions: [
      { title: "统一下单与渠道适配", detail: "商户通过统一接口创建支付单并获得收银台地址；平台根据商户开通能力、支付方式和客户端环境，路由到沃支付、微信或支付宝等适配器。" },
      { title: "订单状态与售后规则", detail: "以待支付、已支付、已关闭和已退款等状态管理交易；撤销仅允许当日已支付且未退款订单，退款支持多次部分退款且累计金额不得超过实付金额。" },
      { title: "回调幂等与商户通知", detail: "以通道和通道流水号作为回调幂等键，验签通过后才更新订单；支付成功后向商户异步通知并记录结果，避免重复回调造成重复记账。" },
      { title: "运营与商户分角色查询", detail: "运营侧提供交易明细、对账差异和交易统计，商户侧只查询自身交易与汇总数据；对账按支付单、通道流水和金额匹配，显性呈现差异。" },
    ],
    outcome: ["完成覆盖收银台、交易售后、运营查询、对账差异与商户门户的可点击交互原型", "形成 PRD、接口契约和包含支付订单状态、通道适配与回调处理的 MVP 工程", "以统一接口、订单状态和对账视图完整演示多渠道支付从下单到售后核对的业务闭环"],
    reflection: "聚合支付的难点不只是把多个支付方式放在一个收银台，而是要在统一体验背后处理渠道差异、订单状态、回调幂等、售后边界和资金数据一致性。",
    tags: ["聚合支付", "收银台", "支付回调", "对账"],
    portfolio: [
      { title: "商户一次接入与聚合收银台", detail: "商户通过统一下单接口创建支付单，平台按开通能力和客户端环境展示可用支付方式，并由渠道适配器完成路由。" },
      { title: "订单状态、回调与售后规则", detail: "以订单状态承接支付、关闭、退款和撤销；通过验签、幂等键和商户异步通知控制支付结果处理。" },
      { title: "交易查询与对账差异处理", detail: "运营侧处理交易明细、差异和统计，商户侧按数据权限查询自身订单；对账将支付单、通道流水和金额进行匹配。" },
    ],
    evidence: [
      { label: "体验交互原型", href: "/prototypes/aggregate-payment/", kind: "document" },
    ],
    media: [
      { src: "/projects/aggregate-payment/prototype-preview.png", alt: "中国联通聚合支付平台收银台原型", caption: "原型覆盖多渠道收银台，并可继续体验交易、对账差异、统计和商户查询。" },
    ],
    variant: "case",
  },
  {
