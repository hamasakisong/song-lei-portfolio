...
const projectRecords: ProjectRecord[] = [
  {
    ...
    order: 1,
    title: "电子券营销平台",
    eyebrow: "0→1 平台建设 × 省分营销",
    summary: "从 0→1 建设电子券全生命周期管理平台，围绕省分营销活动运营效率提升，覆盖活动创建、规则配置、电子券发放、消费核销、资金结算与数据统计。",
  ...
  },
  {
    ...
    title: "沃支付聚合支付平台",
    eyebrow: "支付底层链路 × 自动对账",
    summary: "面向省分业务与内部系统建设的聚合支付能力平台，覆盖统一支付、动态收银、交易售后、运营查询、商户查询与对账差异处理。",
    problem: "不同支付渠道的接口与业务规则存在差异，业务系统若分别直接对接多个渠道，容易形成较高的接入与维护成本；随着渠道增加，渠道账单和平台交易数据还可能因网络异常、跨日交易和渠道处理差异产生不一致，需要统一的支付能力、交易管理与对账机制。",
    role: "担任产品经理，负责支付流程梳理、需求分析和产品设计，完成统一支付、渠道接入、交易售后和对账平台等核心能力规划，并推进 PRD、交互原型、接口契约与工程验证落地。",
    constraints: ["向省分业务和内部系统提供标准化支付接口", "通过渠道适配层屏蔽各支付渠道的交易、环境与接口差异", "自动获取渠道对账文件、解析入库并匹配平台交易，仅将异常差异纳入人工复核"],
    decisions: [
      { title: "统一支付模块", detail: "对外提供标准化支付接口；对内通过渠道适配层完成参数转换、接口调用和支付状态映射，使业务系统无需分别关注各渠道的具体差异。" },
      { title: "动态收银与渠道接入", detail: "将沃支付、微信和支付宝接入统一交易模型；收银台根据订单可用渠道和客户端环境动态展示支付方式，例如微信环境下隐藏支付宝，并映射对应支付场景。" },
      { title: "交易售后与数据权限", detail: "以订单状态承接支付、撤销和部分退款等流程；运营侧支持交易筛选、导出与日报统计，商户侧只查询自身订单，关键操作保留审计记录。" },
      { title: "对账平台", detail: "独立设计自动化对账平台，按日获取渠道对账文件并完成解析入库；系统以支付单号优先、通道流水辅助匹配平台交易，仅将本地单边、渠道单边、金额不符和状态不符等异常差异纳入人工复核与状态流转。" },
    ],
    outcome: ["完成沃支付、微信和支付宝三渠道动态收银，以及支付、退款、撤销、查询和导出的完整交易链路", "完成运营人员、商户运营员和管理员的角色隔离，覆盖交易、报表、对账差异与操作审计", "实现渠道对账文件自动获取、解析入库与交易匹配，仅对异常差异进行人工复核，形成支付与对账闭环"],
    reflection: "聚合支付的重点不只是把多个支付方式汇集到一个入口，而是通过统一支付接口、渠道适配、状态规则和对账机制，把渠道差异、交易状态和账务数据纳入可管理、可追溯的业务闭环。",
    tags: ["聚合支付", "统一支付", "交易售后", "对账平台"],
    portfolio: [
      { title: "动态收银与渠道适配", detail: "订单按可用渠道和客户端环境展示支付方式，并通过适配层承接沃支付、微信和支付宝的调用与状态差异。" },
      { title: "交易售后与角色隔离", detail: "以订单状态管理支付、撤销和多次部分退款；运营与商户按角色分别查询、导出与处理，关键行为进入审计。" },
      { title: "渠道账单自动对账", detail: "系统自动获取渠道对账文件并解析入库，按支付单号和通道流水匹配交易；仅将本地单边、渠道单边、金额不符和状态不符等异常差异交由人工复核。" },
    ],
    evidence: [
  ...
  },
  {
    ...
    title: "房产用户信息管理系统",
    eyebrow: "跨行业建模 × 节点化流程",
    summary: "面向房地产销售业务的信息管理系统：以房号档案为核心，关联购房人、购房协议、资料、付款与网签、交付、房产证领取等流程节点，将线下协作转化为可追踪的系统流程。",
